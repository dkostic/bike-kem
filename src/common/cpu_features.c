...
uint32_t is_avx2_enabled() { return avx2_flag; }
uint32_t is_avx512_enabled() { return avx512_flag; }
uint32_t is_pclmul_enabled() { return pclmul_flag; }
uint32_t is_vpclmul_enabled() {return vpclmul_flag; }
...
#define BIT_AVX2    (1 << 5)
#define BIT_AVX512  (1 << 16)
//#define BIT_PCLMUL  (1 << 5)
#define BIT_VPCLMUL (1 << 10)

static uint32_t get_cpuid_count(uint32_t leaf, uint32_t sub_leaf, uint32_t *eax,
...
void cpu_features_init()
{
  ...
  avx2_flag    = ebx & BIT_AVX2;
  avx512_flag  = ebx & BIT_AVX512;
  pclmul_flag  = avx2_flag;
  vpclmul_flag = ecx & BIT_VPCLMUL;
}
