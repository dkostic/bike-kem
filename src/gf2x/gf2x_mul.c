...
_INLINE_ void gf2x_sqr(OUT dbl_pad_r_t *c, IN const pad_r_t *a)
{
  if(is_vpclmul_enabled()) {
    gf2x_sqr_vpclmul(c, a);
  } else if(is_pclmul_enabled()) {
    gf2x_sqr_pclmul(c, a);
  } else {
    gf2x_sqr_portable(c, a);
  }
}
...
{
  // TODO: add comment
  if(is_vpclmul_enabled() && (qwords_len <= GF2X_BASE_QWORDS_VPCLMUL)) {
    gf2x_mul_base_vpclmul(c, a, b);
    return;
  }
  if(is_pclmul_enabled() && (qwords_len <= GF2X_BASE_QWORDS_PCLMUL)) {
    gf2x_mul_base_pclmul(c, a, b);
    return;
  }
  if(qwords_len <= GF2X_BASE_QWORDS_PORTABLE) {
    gf2x_mul_base_portable(c, a, b);
    return;
  }
