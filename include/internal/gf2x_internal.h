...
void gf2x_mul_base_port(OUT uint64_t *c,
                        IN const uint64_t *a, IN const uint64_t *b);
...
// GF2X methods struct
typedef struct gf2x_methods_st {
...
void gf2x_mod_mul_with_ctx(OUT pad_r_t *c, IN const pad_r_t *a,
                           IN const pad_r_t *b, IN gf2x_methods *ctx);

_INLINE_ void gf2x_methods_init(gf2x_methods *ctx)
{
#if defined(X86_64)
